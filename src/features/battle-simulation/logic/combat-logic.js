...
function simulateRound(pokemon1, pokemon2) {
  ...
  if (attack1 > defense1) {
    winner = attacker;
    log.push(
      `${attacker.name} (Ataque: ${attack1}) supera la defensa de ${defender.name} (Defensa: ${defense1}).`
    );
    log.push(`¡${defender.name} debilitado!`);
  } else {
    log.push(
      `${attacker.name} (Ataque: ${attack1}) no supera la defensa de ${defender.name} (Defensa: ${defense1}).`
    );
    ...
    if (attack2 > defense2) {
      winner = attacker;
      log.push(
        `${attacker.name} (Ataque: ${attack2}) supera la defensa de ${defender.name} (Defensa: ${defense2}).`
      );
      log.push(`¡${defender.name} debilitado!`);
    } else {
      log.push(
        `${attacker.name} (Ataque: ${attack2}) no supera la defensa de ${defender.name} (Defensa: ${defense2}).`
      );
      winner = speed1 >= speed2 ? pokemon1 : pokemon2;
      log.push(
        `Ningún Pokémon pudo debilitar al otro. El más rápido, ${winner.name}, gana la ronda.`
      );
    }
  }
