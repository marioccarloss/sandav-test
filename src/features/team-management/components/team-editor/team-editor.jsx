...
const PokemonItem = ({ pokemon, onRemove, dragHandleListeners }) => (
  <PokemonItemWrapper>
    ...
    <Button onClick={() => onRemove(pokemon.uniqueId)} variant="danger">
      Quitar
    </Button>
  </PokemonItemWrapper>
);
...
const TeamEditor = ({ onSave, isEditMode }) => {
  ...
  const handleSortByAttack = () => {
    const sorted = [...draft.pokemons].sort((a, b) => {
      const attackA = a.stats.find(
        stat => stat.stat.name === 'attack'
      ).base_stat;
      const attackB = b.stats.find(
        stat => stat.stat.name === 'attack'
      ).base_stat;
  ...
  return (
    <TeamEditorWrapper>
      ...
      >
        <SortableContext
          items={draft.pokemons.map(p => p.uniqueId)}
          strategy={verticalListSortingStrategy}
        >
          <PokemonListContainer>
            {draft.pokemons.length === 0 && (
              <EmptyListMessage>
                Añade Pokémon desde la lista. Arrastra para reordenar.
              </EmptyListMessage>
            )}
      ...
      <FooterButtons>
        <Button onClick={() => setDiscardModalOpen(true)} $variant="danger">
          Descartar
        </Button>
        <Button
          onClick={handleSave}
          disabled={!draft.name || draft.pokemons.length < 1}
          $variant="primary"
        >
      ...
      >
        <p>
          ¿Estás seguro de que quieres descartar los cambios? Esta acción no se
          puede deshacer.
        </p>
      </Modal>
    </TeamEditorWrapper>
